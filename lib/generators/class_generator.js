...
Generator.prototype.checkSettings = function(aryFields, fnDoThere, fnNotThere ) {
    var _this = this;

    if (this.options.settings) {

        var hasFields = true;
        aryFields.forEach(function(field){
            if (typeof _this.options.settings[field] == 'undefined') {
                hasFields = false;
                AD.log('<yellow>warn:</yellow> settings file did not have parameter for field: <yellow>'+field+'</yellow>');
            }
        })

        if (hasFields) {
            fnDoThere(this.options.settings);
            return;
        }
    }
...
Generator.prototype.loadADN = function(pathADN) {
    // load the .adn file from the given pathADN

    if (!fs.existsSync(path.join(pathADN, '.adn'))) {
        AD.log('<red>error:</red> expected .adn file not found at path:'+pathADN);
        AD.log();
        process.exit(1);
    }
    

    var adn = require(path.join(pathADN, '.adn'));
    this._adn = adn;
...
/**
 * @function parseAllowedOptions
 * 
 * step through the provided command line options and send the data
 * to the provided callback (cb) if it is one of the allowedOptions.
 *
 * eg: to specify a series of module names:
 *      appdev command  modules:name1,name2,name3
 *
 *      this.parseAllowedOptions(['modules'], function(key, values){
 *          // key: 'modules'
 ...
 *
 *
 * @param {array} allowedOptions  an array of option names
 * @param {fn} cb  a callback fn to call with the option name, and values
 */
Generator.prototype.parseAllowedOptions = function(allowedOptions, cb) {
    ...
    if ((this.options)
        ...
        this.options.options.forEach(function(option){
            ...
            if (_.indexOf(allowedOptions, key) != -1) {
                
                parts.shift();  // remove the key field
 ...
 *
 */
Generator.prototype.parseTemplates = function(listCopies) {
...
AD.log('templatePath:'+templatePath);
            ...
            recursiveScan(this.templateData || {}, templatePath, null, listCopies);
        }
...
var shouldCopyFile = function(name, listCopies) {
...
var recursiveScan = function( data, templatePath, currPath, listCopies ) {
    ...
    for (var f in files) {
        ...
        if (stats.isDirectory()) {
            ...
            recursiveScan( data, templatePath, path.join(currPath, fileName), listCopies );
...
Generator.prototype.perform = function () {
        ...
        // verify .adn exists in closest Plugin or Sails root directory
        this.verifyADN(function(){
            ...
            // the templates
            _this.prepareTemplateData();


            // get template files
            _this.parseTemplates();
...
Generator.prototype.methodStack = function( stack, cb) {
    ...
    var recurseProcess = function(index, list, done) {
        ...
        if (index >= list.length) {
            ...
            if ((list[index]) && (self[list[index]])) {
                self[list[index]](function(err){
                    
                    if (err) {
                        // An error was passed in from the callback.
                        AD.log(err);
                ...
                AD.log('<yellow> ! unknown method ['+list[index]+'].</yellow> Skipping.');
                recurseProcess(index+1, list, done);
            }
        }
    };
    ...
    if (this.atRoot) {
        ...
        var recursiveSearchUP = function( searchFor, limit, current) {
            ...
            if (!fs.existsSync(path.join(currDir , searchFor))) {
                ...
                // move up a level
                process.chdir('..');
                return recursiveSearchUP(searchFor, limit, current+1);
        ...
        if (typeof adn.toRoot == 'undefined') {
            ...
            AD.log('     but there was no toRoot parameter defined. ');
...
/**
 * makes sure our closest Plugin or Sails directory has an .adn file setup.
 *
 * this makes sure that legacy generators that depended upon the existance of the .adn file
 * to determine their start location.
 *
 * @param {String} filename  [optional] file in root directory to search for.
 */
Generator.prototype.verifyADN = function(done) {

//    var self = this;

    // Both our Plugin and Sails root directories should have these directories:
    var aRootDir = {
        'api':1,
        'assets':1,
        'config':1,
    ...
    if (this.atRoot) {
        ...
        // what file to search for
        var adnFileName = '.adn';   // default: look for our .adn file in root of directory.


        // function to step up wards in our directory structure to find our root
        var recursiveSearchUP = function( limit, current) {
            ...
            // if we didn't find our searchfor file in our current directory
            if (!AD.util.fs.looksLikeDir(aRootDir, currDir)) {

                // move up a level
                process.chdir('..');
                return recursiveSearchUP(limit, current+1);
        ...
        var foundRoot = recursiveSearchUP();
        ...
        if (!fs.existsSync(path.resolve(process.cwd(), adnFileName))) {

            // our .adn file doesn't exist, so create it:
            AD.spawn.command({
                command:'appdev',
                options: ['adn']
            })
            .fail(function(err){
                AD.log.error('<bold>error:</bold> could not create a default .adn file');
                process.exit(1);
            })
            .then(function(){
                done();
