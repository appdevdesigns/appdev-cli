...
Module.help = function() {
    ...
    this.showMoreHelp(    {
        commandFormat:'appdev plugin <green><bold>[pluginName]</bold></green>',
        description:[ 
            'This command generates a new npm plugin for a sails project.',
            '',
            '    This command is expected to be run in the <yellow>'+path.join('sailsRoot', 'node_modules')+'</yellow> directory.',
            ''

        ].join('\n'),
        parameters:[
            '<green>[pluginName]</green>  :   the new name for the plugin. '
        ],

        examples:[
            '> appdev plugin <green>spiffyPlugin</green>'
        ]
    });
...
Module.prepareTemplateData = function () {
    ...
    this.templateData.moduleName = this.templateData.moduleName.toLowerCase();
    ...
    if ( fs.existsSync(pathToModule)) {
        ...
        AD.log("<yellow>use this command to only create new plugins.</yellow>");
        AD.log();
...
Module.installDependencies = function(done) {
    ...
    var listDependencies = [
        ...
        // > npm install Barrels --save-dev
        { command:'npm', options:[ 'install',  'barrels', '--save-dev'], textFilter:textFilters, log:'<green><bold>installing:</bold> Barrels (dev: fixtures)</green>', shouldEcho:false },

        // > npm install Supertest --save-dev
        { command:'npm', options:[ 'install',  'supertest', '--save-dev'], textFilter:textFilters, log:'<green><bold>installing:</bold> Supertest (dev: controllers)</green>', shouldEcho:false },
    ...
    if (this.templateData.clientTests) {
        ...
        listDependencies.push({ command:'npm', options:[ 'install',  'phantomjs@1.9.7-15', '--save-dev'], textFilter:textFilters, log:'<green><bold>installing:</bold> phantomjs (dev: unit testing client)</green>', shouldEcho:false });
...
Module.npmQuestions = function(done) {
            ...
            then:[
                ...
                },
                {
                  ...
                  post:function(data) {
                      ...
                      // this local data.gitRepo is going into the package.json file
                      // and needs to be the long git url.
                      ...
                      // save this to our templatesData.gitRepo
                      // which should only be:  [gitAccount]/[project]
                      // this goes into our .travis.yml  file
    ...
    this.questions(qset, function(err, data) {
        ...
        if (err) {
            ...
            // BUG FIX: NPM no longer allows capital letters in the 'name'
            var lcModuleName = self.templateData.moduleName.toLowerCase();
            var responses = {
                    'name:':lcModuleName+'\n',
