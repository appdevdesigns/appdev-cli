...
Resource.prepareTemplateData = function () {
    ...
    var allowedOptions = [ 'connection', 'tablename' ];
            ...
            if ( templateTypes.indexOf(type) == -1) {
                ...
                switch(type) {
                    ...
                    case 'model':
                        ...
                        this.templateData.templateFields.push('    '+field+" : { model: '"+parts[2]+"' },");
                        this.templateData.associations.push(field);
                        break;
                    ...
                    case 'collection':
                        ...
                        this.templateData.templateFields.push('    '+field+" : {  collection: '"+parts[2]+"', via: '"+parts[3]+"' },");
                        this.templateData.associations.push(field);
                        break;
                }
    ...
    // prepare the description for the Client UI Model
    // this.templateData.description = JSON.stringify(desc, null, 22);
    this.templateData.description = "{ " + desc.join(', ') + ' }';

    // prepare the multilingualFields for the Client UI Model
    ...
    // prepare the clientAssociations for the client UI model definition:
...
Resource.patchFiles = function(done) {
    ...
    // if multilingual, update the Data Model with the additional Translation attributes
    if (hasMultilingual) {
