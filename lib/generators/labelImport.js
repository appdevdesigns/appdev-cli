...
Resource.help = function() {
    ...
    this.showMoreHelp(    {
        commandFormat:'appdev labelImport <green>[dir:[path/to/file]] [format:[type]]</green>',
        description:[ 
            'This command imports all the multilingual labels for this application.',
            '',
            '    By default we look for labels in the '+path.join('setup','labels')+' directory of the current application.',
            '',
            '    You can select which label formats to import.  The default format is <yellow>.po</yellow> . ',
            ''
        ].join('\n'),
        parameters:[
            '<green>[dir:[path/to/file]]</green>   :   the path of the directory to import labels from',
            '<green>[format:[type]]</green>       :   which format to import?  [ .po, .json] '
        ],
...
var createLabel = function(self, alabel, cb) {
    ...
    alabel.label_needs_translation = 1;
    alabel.createdAt = moment().format();
...
var updateLabel = function(self, alabel, cb) {
    ...
    var sql = "UPDATE "+self.dbInfo.database+'.'+DB_TABLE_LABELS 
            + ' SET label_label="'+alabel.label_label.replace('"', '\"')+'", updatedAt="'+moment().format()+'" '
            + ' WHERE id="'+alabel.id+'"';
