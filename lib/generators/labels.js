...
Resource.checkSettings = function(done) {
    ...
    // 1st check to see what adaptor is set.  If not mysql, then 
    // we should exit for now. We have no way to modify the label
    // currently.
    var adapter = this.defaults('adapter');
    if (adapter && adapter.toLowerCase && adapter.toLowerCase() == 'mysql') {
...
//// TODO: instead of exiting, then lift sails and use the Sails Models
////       slower, but will still work.

        AD.log("<yellow> requires mysql adapter for label import. </yellow>");
        process.exit(0);
    }
