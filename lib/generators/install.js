...
Resource.help = function() {
    ...
    this.showMoreHelp(    {
        commandFormat:'appdev install <yellow><bold>[dir_name]</bold></yellow> <green>[plugins:moduleA,moduleB] [settings:path/to/settings.js] [--develop] [--travisCI]</green>',
        description:[ 
    ...
    if (this.params.develop) {
        ...
        var recursiveInstall = function(list) {
            ...
            if (list.length == 0) {
                ...
                AD.spawn.command({
                    command:'appdev',
                    options:[ 'labelInstall' ]
                })
                .fail(function(err){
                    AD.log.error('<bold>develop:</bold> error installing labels in module ['+module+']', err);
                    process.exit(1);
                })
                .then(function(code){
