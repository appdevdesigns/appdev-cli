...
var Resource = new Generator({
    ...
    additionalOptions:[ 
        ...
        { key:'--AppBuilder', desc:'setup a fresh AppBuilder install'},
...
Resource.help = function() {
    ...
    this.showMoreHelp(    {
        ...
        parameters:[
            ...
            '<green>--AppBuilder</green>       :   <green>(optional)</green> install a fresh configured AppBuilder instance.',
...
Resource.perform = function () {
    ...
    if (this.params.AppBuilder) {
AD.log("<yellow>--AppBuilder detected.</yellow>");
        this.params.AppBuilder  = true;
        this.params.develop = true;     // make sure our --develop flag is also included.
    }
...
// process.exit(0);
    ...
    if (this.options.dirName) {
        ...
        var setupProcess = [
                        ...
                        'developSailsLift',         // have sails create the initial DB tables
                        'developActionKeys',        // install the action keys for the core modules
                        'developPermissions',       // create default admin user and permissions
                        'developCoreLabels',        // install the labels for the core files
                        'developCreateOpsPortal',   // create the page/opsportal holder


                        'appBuilderInstall',        // make sure the AppBuilder is setup
...
Resource.installDBAdaptor = function (done) {
    ...
    if (this.params.travisCI) {
        ...
        function doAdaptor (data) {
            ...
            // sanity check:
            var validAdaptors = ['sails-mysql', 'mysql', 'sqllite', 'memory', 'sails-disk', 'disk'];
            if (validAdaptors.indexOf(data.adaptor) == -1) {
                AD.log.error('Invalid adaptor specified:'+ data.adaptor);
                AD.log.error('please choose one of the following:');
                AD.log(validAdaptors.join('\n'));
                process.exit(1);
            }
...
Resource.installTestDBAdaptor = function (done) {
    ...
    if ((this.params.travisCI) ||  (this.params.docker)) {
...
Resource.installLibraries = function(done) {
    ...
    // this makes sure the .adn is created:
    this.defaults('installLibs', 'Y');

    var textFilters = ['Creating', 'GET', 'npm', 'http', '─', '> ', '304', '200', 'WARN'];

    var listInstall = [ ];

    // only install a default client side application if the given .dirName isn't one of our
    // protected Directories:
    // NOTE:  performing case insensitive match:
    var protectedDirectories = [ 'opsportal' ];
    if (protectedDirectories.indexOf(this.options.dirName.toLowerCase()) == -1 ) {

            // use appdev application [dirName] to setup our application inside the framework
            listInstall.push({ command:'appdev', options:[ 'application',  this.options.dirName], textFilter:textFilters, log:'<yellow><bold>install:</bold></yellow> setting up client directories for '+this.options.dirName });
    }
    ...
    if (!this.params.noDependencies) {
        ...
        if (this.params.AppBuilder) {
            ...
            modules.push('appdevdesigns/app_builder'+branchTag);
...
Resource.installMemoryAdaptor = function (done) {
    ...
    this.defaults('adapter', 'memory'); // mark that we are using memory
...
Resource.installDiskAdaptor = function (done) {
    ...
    this.defaults('adapter', 'disk'); // mark that we are using disk
...
Resource.installMysqlAdaptor = function (done) {
    ...
    function doInstall (data) {
        ...
        for (var f in fieldHash) {
            var v = fieldHash[f];
// console.log('... defaults:'+f+', '+data[v]);
            _this.defaults(f,data[v]);
        }
        _this.defaults('adapter', 'mysql'); // mark that we are using mysql
...
//                             {  file:path.join('assets', 'js', 'app.js'), tag:/(socket\.on\('message'[\s\S]+?)(\}\);)/, replace:adPublish }
        ...
        _this.patchFile( patchSet, function() {
            ...
            // now install the sails-mysql adaptor
            // NOTE: don't install from github: use the npm version:
            // var params = [ 'install',  'balderdashy/sails-mysql', '--save'];

//// TODO: currently we are patching sails-mysql & waterline-sequel with our own { and:[] } conditon patch.
////       once there is a fix in the waterline-sequel project, then return to the main sails-mysql:
//// @see: https://github.com/balderdashy/waterline-sequel/pull/68
            // var params = [ 'install',  'sails-mysql', '--save'];
//// until then, use our fork:
var params = [ 'install',  'appdevdesigns/sails-mysql#0.12.x', '--save'];
...
Resource.createDefaultDirectories = function(done) {
    ...
    function checkPath(list, cb) {
        ...
        if (list.length == 0) {
            ...
            var actualPath = path.join(cwd, currPath);
...
/*
    ...
    *** {
    "message": "The hook `orm` is taking too long to load.\nMake sure it is triggering its `initialize()` callback, or else set `sails.config.orm._hookTimeout to a higher value (currently 20000)",
    "stack": [
        "Error: The hook `orm` is taking too long to load.",
        "Make sure it is triggering its `initialize()` callback, or else set `sails.config.orm._hookTimeout to a higher value (currently 20000)",
        "    at Timeout.tooLong [as _onTimeout] (/Users/codingMonkey/.npm-packages/lib/node_modules/sails/lib/app/private/loadHooks.js:85:21)",
...
// we need to make sure our Action Keys are installed:
    ...
    if ((this.params.develop) || (this.params.docker)) {
        ...
        // if --AppBuilder was specified, make sure we add that one too:
        if (this.params.AppBuilder) {
            listModules.push('app_builder');
        }
...
// go ahead and setup the permissions:
    ...
    if ((this.params.develop) || (this.params.docker)) {
        ...
        var doPermissions = function(data) {
            ...
            var responses = {
                ...
                'scope of all users:': '\n',
                'production': 'dev\n',
                'languages' : data.installLanguages+'\n',
...
//                shouldEcho:false,
        ...
        if (!this.params.travisCI) {
            ...
            this.checkSettings( ['installLanguages'], doPermissions, function() { 

                var qset =  {
                    question: 'which languages do you want installed by default [en:English,ko:Korean,th:Thai,zh-hans:Chinese]:',
                    data: 'installLanguages',
                    def : 'en:English,ko:Korean,zh-hans:Chinese,th:Thai'
                };
                _this.questions(qset, function(err, data) {
                    ...
                    if (err) {
                        ...
                        // Badly formed language strings can result in 
                        // infinite recursion. If we can't parse the string 
                        // here, just use the default.
                        
                        var isValid = false;
                        var langs = String(data.installLanguages).split(',');
                        if (langs.length > 0) {
                            isValid = true;
                        }
                        ...
                        if (!isValid) {
                            AD.log.error('Unable to parse that language string. Using default instead.');
                            data.installLanguages = qset.def;
                        }
            ...
            doPermissions({ installLanguages:'en:English'});
        }
...
Resource.developCoreLabels = function(done) {
// in the setup process, if a --develop command is given,
// make sure our core modules have their labels installed:
    var _this = this;
    ...
    // if we are in --develop mode
    if (!(this.params.travisCI)
         && ((this.params.develop) || (this.params.docker)) ) {

        AD.log('<yellow>develop:</yellow> installing core labels');
        var currPath = process.cwd();

        // perform a labelInstall for these modules:
        var modules = ['appdev-opsportal'];

        // if --AppBuilder specified, make sure it is included
        if (this.params.AppBuilder) {
            modules.push('app_builder');
        }

        var recursiveInstall = function(list) {
            ...
            if (list.length == 0) {
                ...
                process.chdir(path.join('node_modules',module));
                ...
                .fail(function(err){
                    AD.log.error('<bold>develop:</bold> error importing labels in module ['+module+']', err);
                    process.exit(1);
                })
                .then(function(code){

                    process.chdir(currPath);
                    recursiveInstall(list);

                });
            }
        }

        recursiveInstall(modules);
...
// This should now be done in: appdev page opsportal                    
            ...
            // 3) make the default route our page/opsportal
            function(next) {
                ...
                var patchSet = [
                    ...
                    // make the default route our page/opsportal
                    {
                        file: path.join('config', 'routes.js'),
                        tag: /'\/'\s*[^}]*}/,
                        replace: "'/': 'PageController.opsportal'"
                    }
                ];
                _this.patchFile(patchSet, function() {
                    next();
...
Resource.appBuilderInstall = function(done) {
// in the setup process, if a --AppBuilder command is given,
// go ahead and run its install.js :
    var _this = this;

    // if we are in --AppBuilder mode
    if (this.params.AppBuilder) {

        var currPath = process.cwd();


        process.chdir(path.join('node_modules','app_builder','setup', 'install'));

        // install.js => sets up an initial AppBuilder entry in the system.
        AD.spawn.command({
            command:'node',
            options:[ 'install.js' ],
//                shouldEcho:false,
...
//         //Fix Local for docker base plugin
//         //Add Connection Here
//         var data = {};

//         if (this.options.settings) {
//             data.connectionType = this.options.settings.connectionType;
//             data.host = this.options.settings.host;
//             data.port = this.options.settings.port;
//             data.user = this.options.settings.user;
//             data.password = this.options.settings.password;
//             data.database = this.options.settings.database;
//         } 
//         else {
//             //Default Value
...
//         var patchSet = [ {  file:path.join('config', 'local.js'), tag:/ "fcf"*:\s*\{[^}]*\} */g, template:'__config_fcf.ejs', data:data  }
//         ];
