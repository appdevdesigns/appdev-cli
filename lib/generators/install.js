...
Resource.help = function() {
    ...
    this.showMoreHelp(    {
        commandFormat:'appdev install <yellow><bold>[dir_name]</bold></yellow> <green>[--develop] [--travisCI] [--docker]</green>',
        description:[ 
...
// AD.log('<green><bold>****** --develop mode ******</bold></green>');
        ...
        listInstall.push({ command:'npm', options:[ 'install',  'steal', '--save'], textFilter:textFilters, log:'<yellow><bold>install:</bold></yellow>  stealjs as a dependency', retry:3 });
...
//         listInstall.push({ command:'rm',    options:[ '-R', path.join( 'assets', 'javascriptmvc.zip')], textFilter:textFilters, log:'<yellow><bold>install:</bold></yellow>  rm: javascriptmvc', retry:3 });
    ...
    .then(function(){
        ...
        var stealAssetPath = path.join('assets', 'steal', 'steal');
