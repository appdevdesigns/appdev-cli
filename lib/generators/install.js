...
Resource.perform = function () {
    ...
    if (this.options.dirName) {
        ...
        if (fs.existsSync(this.options.dirName) )  {
            ...
            AD.log('<yellow>Use this command for new installations only.</yellow>');
            AD.log();
            process.exit(1);
...
//            { command:'git', options:[ 'clone',  'https://github.com/appdevdesigns/appdev-client.git', path.join('assets', 'appdev')], textFilter:textFilters, log:'<yellow><bold>install:</bold></yellow>  appdev client side library', retry:3 },
            ...
            { command:'npm', options:[ 'install',  'appdevdesigns/appdev-core#v10', '--save'], textFilter:textFilters, log:'<yellow><bold>install:</bold></yellow>  appdev-core as a dependency', retry:3 },

            // install async :  our plugin modifications to config/bootstrap.js requires this.
            { command:'npm', options:[ 'install',  'async', '--save'], textFilter:textFilters, log:'<yellow><bold>install:</bold></yellow>  async as a dependency', retry:3 },

            // install ad-utils :  our plugin modifications to config/bootstrap.js requires this.
            { command:'npm', options:[ 'install',  'appdevdesigns/ad-utils#v10', '--save'], textFilter:textFilters, log:'<yellow><bold>install:</bold></yellow>  ad-utils as a dependency', retry:3 },
...
//    var self = this;
    ...
    AD.log('<yellow><bold>disable:</bold> *** disabling Grunt File: </yellow>');

    var params = [ 'Gruntfile.js',  'Gruntfile_disable.js'];
    ...
    .fail(function(err){
        AD.log.error('<bold>error:</bold> unable to disable grunt file: \'mv '+params.join(' ')+'\'');
        process.exit(1);
    })
    .then(function(){
