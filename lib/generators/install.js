...
Resource.installSailsApp = function (done) {
    ...
    this.shell('sails', params, [], function(){
        ...
        // Sails includes an isAuthenticated.js policy we want to remove.
        fs.unlinkSync(path.join('api', 'policies', 'isAuthenticated.js'));
...
//            { cmd:'git', params:[ 'clone',  'ssh://gitolite@git.zteam.biz/appdev_client', path.join('assets', 'appdev')], filter:['Creating'], log:'installing appdev client side library' },
            ...
            { cmd:'npm', params:[ 'install',  'appdevdesigns/appdev-core', '--save'], filter:['Creating', '─', '> '], log:'installing appdev-core as a dependency', retry:3 },

            // install async :  our plugin modifications to config/bootstrap.js requires this.
            { cmd:'npm', params:[ 'install',  'async', '--save'], filter:['Creating'], log:'installing async as a dependency', retry:3 },

            // install ad-utils :  our plugin modifications to config/bootstrap.js requires this.
            { cmd:'npm', params:[ 'install',  'appdevdesigns/ad-utils', '--save'], filter:['Creating'], log:'installing ad-utils as a dependency', retry:3 },
