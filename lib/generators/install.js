...
var Resource = new Generator({
    ...
    parameters:['dirName'],
    additionalOptions:[ { key:'--noDependencies', desc: 'do not install any dependencies for this action'}, { key:'--develop', desc:'setup in development mode'}]
});
...
AD.log('<green><bold>****** --develop mode ******</bold></green>');
        ...
        // also install the opsportal & hrisTools in development mode:
        if (this.params.develop) {
            listInstall.push({ command:'npm', options:[ 'install',  'appdevdesigns/appdev-opsportal'+branchTag, '--save'], textFilter:textFilters, log:'<yellow><bold>install:</bold></yellow>  appdev-opsportal'+branchTag+' as a dependency', retry:3 });
            listInstall.push({ command:'npm', options:[ 'install',  'appdevdesigns/opstool-hrisAdminObjects'+branchTag, '--save'], textFilter:textFilters, log:'<yellow><bold>install:</bold></yellow>  appdev-hrisAdminObjects'+branchTag+' as a dependency', retry:3 });
            listInstall.push({ command:'npm', options:[ 'install',  'appdevdesigns/opstool-hrisUserProfile'+branchTag, '--save'], textFilter:textFilters, log:'<yellow><bold>install:</bold></yellow>  appdev-hrisUserProfile'+branchTag+' as a dependency', retry:3 });
        }
...
Resource.setModelMigrate = function (done) {
    ...
    //// To be safe, we are going to set the 
    //// // migrate: 'alter'  
    //// to 
    //// migrate: 'safe'
    ////
