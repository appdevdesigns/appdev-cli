...
ViewUI.help = function() {

    this.showMoreHelp(    {
        commandFormat:'appdev viewUI <yellow><bold>[path/to/view.ejs]</bold></yellow>',
        description:[ 
            'This command creates a new CanJS client side view template.',
            '',
            '    The template will be at '+path.join('assets', 'path','to','view.ejs')+'.'

        ].join('\n'),
        parameters:[
            '<yellow>[path/to/view.ejs]</yellow>         :   the path and name of the view you want to make.'
        ],

        examples:[
            '> appdev viewUI opstools/MyWidget/views/WidgetList/NewView.ejs',
            '    // creates '+path.join('assets', 'opstools','MyWidget', 'views', 'WidgetList','NewView.ejs'),
            '',
        ]
    });
