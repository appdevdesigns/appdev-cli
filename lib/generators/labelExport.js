...
Resource.getLabels = function(done) {
    ...
    async.series([
        ...
        // step 1:  find the ids of the destination labels
        function(next) {

            var sql = "select * from "+self.dbInfo.database+'.'+DB_TABLE_LABELS;
            sql += ' where label_context LIKE \'%'+self.templateData.application+'%\'';
            sql += ' AND language_code="'+self.templateData.dest+'" ';

            if (self.templateData.nottranslated) {
                // make sure the destination language labels needs translation
            ...
            self.db.query(sql,function(err,results){
                ...
                if (err) {
                    AD.log();
                    AD.log.error('> error selecting dest labels from '+self.dbInfo.database+'.'+DB_TABLE_LABELS+' table:',err,sql);
                    AD.log();
                    next(err);
                } else {

                    // save the ids of these labels:
        ...
        // step 2:  find the ids of the corresponding source labels
        function(next) {

            var sql = "select * from "+self.dbInfo.database+'.'+DB_TABLE_LABELS;
            sql += ' where label_context LIKE \'%'+self.templateData.application+'%\'';
            sql += ' AND language_code="'+self.templateData.source+'" ';
            sql += ' AND label_key IN (';

            var allKeys = [];
            destRows.forEach(function(row){
                allKeys.push( "'"+row.label_key+"'" );
            })

            if (allKeys.length == 0) {

                AD.log();
                AD.log(' There were no label found.');
                AD.log();

                var err = new Error('no labels found');
                ...
                self.db.query(sql,function(err,results){
                    ...
                    if (err) {
                        AD.log();
                        AD.log.error('> error selecting source labels from '+self.dbInfo.database+'.'+DB_TABLE_LABELS+' table:',err,sql);
                        AD.log();
                        next(err);
                    } else {
        ...
        // step 3:  gather all labels and compile
        function(next) {
...
//     // only include labels in the two languages (source/dest) we are looking at
//     sql += ' AND (language_code="'+this.templateData.source+'" OR language_code="'+self.templateData.dest+'") ';


//     if (this.templateData.nottranslated) {

//         // make sure the destination language needs translation
//         // sql += ' AND (language_code="'+self.templateData.dest+'" AND label_needs_translation=1)';
//         sql += ' AND label_needs_translation=1';
//     }
// AD.log('... sql:'+sql);
...
//TODO:  is there an error when the /setup/labels/  directory doesn't already exist?

    var filePath = path.join(process.cwd(), 'setup', 'labels', name);
// console.log('... writing to file:'+filePath);

    fs.writeFileSync( filePath, this.templateContents, 'utf8');
    AD.log('... saved file:<yellow>'+filePath+'</yellow>');
