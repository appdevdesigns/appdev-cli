...
Resource.help = function() {
    ...
    this.showMoreHelp(    {
        commandFormat:'appdev labelExport <yellow><bold>[application]</bold></yellow> <green>[source:[langCode]] [dest:[langCode]] [format:[type]]</green>',
        description:[ 
            ...
            '',
            '    The label files will be named <green>labels_[dest_langCode].[format]</green>.',
            '',
...
// AD.log('... options:', this.options);
    ...
    this.options.options.forEach(function(option){
        ...
        if (self.templateData[key]) {

            // this matches one of our defaults

            // if this is one of the lang options:
            if (key != 'format') {
